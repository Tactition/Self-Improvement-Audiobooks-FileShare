...
async def start(client, message):
    # New: Check if a parameter is provided for text deep links.
    if len(message.command) > 1:
        ...
        if param.startswith("text_"):
            ...
            # For other parameters, continue with your normal /start logic.
            pass
        ...
        if AUTO_DELETE_MODE == True:
            ...
            for x in filesarr:
                try:
                    ...
                    pass
            await k.edit_text("<b>File deleted successfully. You are always welcome to request again</b>")
        return
    ...
    if not files_:
        ...
        try:
            ...
            if AUTO_DELETE_MODE == True:
                k = await client.send_message(chat_id=message.from_user.id, text=f"<b><u>❗️IMPORTANT❗️</u></b>\n\n This File will be deleted Within <b><u>{AUTO_DELETE} Minutes</u></b>  <i>(Due to Copyright Issues)</i>.\n\n<b>So, You Are Requested to Forward The File to Saved Messages </b>")
                await asyncio.sleep(AUTO_DELETE_TIME)
                try:
                    await msg.delete()
                except:
